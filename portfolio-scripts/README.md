...
- `01-start-working.ps1` — opens `D:\Portfolio`, safely recovers from an unfinished Git operation or misplaced uncommitted work, switches to `develop`, and pulls with fast-forward only.
...
- `05-publish-production.ps1` — asks you to confirm the develop version and enter the merge commit message, verifies the matching final-test record, merges into `main`, pushes, creates the matching annotated tag, and returns to `develop`.
...
- No broad automatic merge-conflict resolution. During publishing, the script can resolve only `package.json`/`package-lock.json` conflicts by keeping the tested `develop` copies, and only after asking you.
- Uncommitted work found outside `develop` can be transferred through a named backup stash; the stash is retained after applying it.
- An unfinished merge, rebase, cherry-pick, revert, or bisect is detected before any branch switch. Aborting requires your explicit choice.
- Release preparation requires a clean, synchronized `develop` branch.
- Publishing reads the version from `develop/package.json`, asks you to type the same version again, and rejects any mismatch before switching to `main`.
